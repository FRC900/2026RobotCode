...
public class TalonFXIO implements MotorIO {
    ...
    @Override
    public void setPositionSetpoint(double units, double ffkV) {
        talon.setControl(
                positionTorqueCurrent.withPosition(clampPosition(units)).withVelocity(ffkV));
    }
    ...
    public void setVelocitySetpoint(double unitsPerSecond, int slot) {
        talon.setControl(
velocityTorqueCurrentFOC                        .withVelocity(unitsToRotor(unitsPerSecond))
                        .withSlot(slot));
    }
