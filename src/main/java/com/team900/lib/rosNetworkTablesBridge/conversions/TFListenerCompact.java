...
import com.team900.lib.rosNetworkTablesBridge.Pair;
...
/**
 * The purpose of TFListenerCompact is to subscribe to /tf_compact. The ROS host will publish
 * certain transforms. For example, if the ROS host is configured to publish map to base_link,
 * TFMessages that encode this transform will be received on this topic.
 */
public class TFListenerCompact {
    ...
    /**
     * Constructor initializes the BridgeSubscriber with the given ROSNetworkTablesBridge and topic
     * name
     *
    ...
    /**
     * Updates the lookup table with new transforms received from the /tf_compact topic. Call this
     * before canTransform or lookupTransform.
     */
    public void update() {
        ...
        if ((optMsg = m_tfCompactSub.receive()).isPresent()) {
            ...
            for (TransformStamped tfStamped : msg.getTransforms()) {
                ...
                Pair<String, String> key =
                        new Pair<>(tfStamped.getHeader().getFrameId(), tfStamped.getChildFrameId());

                // Create a Transform3dStamped object with the current time and the WPI
                // transform
                Transform3dStamped transformStamped =
                        new Transform3dStamped(
                                tfStamped.getHeader(), tfStamped.getChildFrameId(), transform);
     ...
     * @return true if a transform exists between the parent and child frame IDs, false otherwise
     */
    ...
    /**
     * Checks if a transform exists between the given parent and child frame IDs within a specified
     * time limit.
     *
     * @param parent_frame_id The parent frame ID to check for a transform
     * @param child_frame_id The child frame ID to check for a transform
     * @param timeLimit The time limit (in seconds) within which the transform should have been
     *     updated
     * @return true if a transform exists between the parent and child frame IDs within the
    ...
    public boolean canTransform(String parent_frame_id, String child_frame_id, double timeLimit) {
        Optional<Transform3dStamped> opt_transform =
                lookupTransform(parent_frame_id, child_frame_id);
        if (opt_transform.isEmpty()) {
            return false;
        }
        Transform3dStamped transform = opt_transform.get();
        return m_tfCompactSub.getNow().toSeconds() - transform.header.getStamp().toSeconds()
                < timeLimit;
    }

    /**
     * Retrieves the Transform3dStamped object for the given parent and child frame IDs.
     *
     * @param parent_frame_id The parent frame ID for which to retrieve the transform
     * @param child_frame_id The child frame ID for which to retrieve the transform
     * @return The Optional Transform3dStamped object corresponding to the given parent and child
     *     frame IDs. If no transform exists between the given parent and child frame IDs, the
     *     Optional will be empty.
     */
    public Optional<Transform3dStamped> lookupTransform(
            String parent_frame_id, String child_frame_id) {
        if (canTransform(parent_frame_id, child_frame_id)) {
            return Optional.of(
                    lookup.get(new Pair<String, String>(parent_frame_id, child_frame_id)));
        }
