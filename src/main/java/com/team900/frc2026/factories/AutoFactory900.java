...
public class AutoFactory900{
    ...
    public static Command resetPose(Pose2d pose) {
        return new InstantCommand(() -> container.getDriveSubsystem().resetPose(pose), 
    ...
    public static Command retractSlapdown() {
        return new ParallelCommandGroup(
            IntakeFactory.retractSlapdown(container), 
            IntakeFactory.exhaustIntake(container)
        );
    }
    ...
    public static Command alignToHub(DoubleSupplier throttle, DoubleSupplier strafe, DoubleSupplier turn) {
