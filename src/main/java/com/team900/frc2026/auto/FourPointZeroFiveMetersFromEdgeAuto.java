...
public class FourPointZeroFiveMetersFromEdgeAuto {
    ...
    public static Command getAutoCommand() {
       ...
       AutoFactory choreoFactory = new AutoFactory(
            container.getDriveSubsystem()::getPose,
            container.getDriveSubsystem()::resetPose,
            (SwerveSample sample) -> {
                double xFB = xController.calculate(
                    container.getDriveSubsystem().getPose().getX(), sample.x);
                double yFB = yController.calculate(
                    container.getDriveSubsystem().getPose().getY(), sample.y);
                double rFB = rotController.calculate(
                    container.getDriveSubsystem().getPose().getRotation().getRadians(),
                    sample.heading);
