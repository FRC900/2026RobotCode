...
def fit_surfaces(npz_path, poly_degree=4):
    ...
    print(f"Outliers removed: {len(r) - keep.sum()} / {len(r)}  ->  {keep.sum()} clean points")
    ...
    print(f"phi R2 = {r2_phi:.6f} (inputs: r, vf)")
    print(f"theta R2 = {r2_theta:.6f} (inputs: vf, vl)")
    return phi_model, theta_model

phi_model, theta_model = fit_surfaces('ProjectileMotionSim/PassingSurfaces/clean_theta_phi_passing_surface.npz')

with open('ProjectileMotionSim/PassingSurfaces/models/phi_passing_model.pkl', 'wb') as f:
    pickle.dump(phi_model, f)

with open('ProjectileMotionSim/PassingSurfaces/models/theta_passing_model.pkl', 'wb') as f:
...
export_model(phi_model, 'ProjectileMotionSim/PassingSurfaces/models/phi_passing_model.json')
export_model(theta_model, 'ProjectileMotionSim/PassingSurfaces/models/theta_passing_model.json')
