...
# load data
data = np.load("ProjectileMotionSim/PassingSurfaces/clean_theta_phi_passing_surface.npz")
r_vals = data['r_vals']
vf_vals = data['vf_vals']
vl_vals = data['vl_vals']
initial_speed_mag = 22.5 # m/s, initial launch speed
theta_surface = data['theta_surface']
phi_surface = data['phi_surface']

# load models
with open('ProjectileMotionSim/PassingSurfaces/models/phi_passing_model.pkl', 'rb') as f:
    phi_model = pickle.load(f)
with open('ProjectileMotionSim/PassingSurfaces/models/theta_passing_model.pkl', 'rb') as f:
